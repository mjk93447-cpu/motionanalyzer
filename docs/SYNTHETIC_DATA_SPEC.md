...
| edge_scorch | 목표 1 | **레이저 컷팅 테두리 그을림 → 접합 약화 → 벤딩 시 최외곽 벌어짐** |
| pre_damage | 목표 2 | 미묘한 물성 차이, 전체 궤적 변화 |

### edge_scorch (신규, 현장 보고 반영)

- **원인**: 레이저 컷팅 시 FPCB 테두리 그을림( scorch) → 패널 접합 약화
- **현상**: 벤딩 시 최외곽 일부가 벌어지는(gape) 패턴
- **모델링**: `edge_gape_gain`, `edge_gape_width_ratio` — 곡률이 양 끝단(edge)에 집중
