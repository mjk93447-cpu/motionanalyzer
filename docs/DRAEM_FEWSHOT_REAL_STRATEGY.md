# DRAEM: 소수 실제 이상 데이터로 정확도·리콜 획기적 향상 전략

## 배경

- **현재**: DRAEM(DRAEM 전략)은 **정상 데이터만**으로 학습하며, 합성 이상(정상+노이즈)으로 판별기를 함께 학습한다.
...
1. **1단계 (현재 완료)**: 정상만 + 합성 이상으로 DRAEM 학습 → 기본 판별기·재구성기 확보.
2. **2단계 (소수 실제 이상 활용)**  
   ...
   - **옵션 C — 메타/프로토타입 보정**  
     ...
     - 추론 시: 이상 점수 = 기존 DRAEM 점수 + (실제 이상과의 유사도) 등으로 보정해 리콜을 끌어올림.
3. **평가**  
...
- **데이터 파이프라인**: 실제 이상 영상 → 기존 `prepare_training_data` / 특징 추출과 **동일 포맷**으로 맞춰 DRAEM 입력과 호환.
...
이 문서는 DRAEM 모드 개발 완료 후, 소수 실제 이상 데이터를 활용한 정확도·리콜 향상의 **전략 기준**으로 사용한다.
