...
def run_fpcb_pipeline(
    ...
    """
    Physics-based FPCB bending: generate high-fidelity dataset -> analyze -> export vector map and metrics plots.
    Generated images: export_vectors_dir/vector_map.png, plots_dir/fpcb_metrics.png
    """
    init_dirs(base_dir=Path("."))
    config = high_fidelity_fpcb_config(
        frames=frames,
        points_per_frame=points_per_frame,
        fps=fps,
        seed=seed,
    )
    console.print("[cyan]1/3[/cyan] Generating high-fidelity FPCB bending dataset (physics-based)...")
    generate_synthetic_bundle(output_dir=data_dir, config=config)
    console.print(f"[green]Generated[/green] {data_dir} (frame_*.txt, fps.txt, frame_metrics.csv, metadata.json)")

    console.print("[cyan]2/3[/cyan] Running motion analysis (vectors, summary, vector_map.png)...")
    summary = run_analysis(input_dir=data_dir, output_dir=export_vectors_dir)
    console.print(
        f"[green]Analysis done[/green] frames={summary.frame_count} mean_speed={summary.mean_speed:.3f} px/s"
    )
    ...
    if metrics_csv.exists():
        console.print("[cyan]3/3[/cyan] Plotting frame metrics (bend angle, curvature, strain)...")
        plot_frame_metrics(metrics_csv, metrics_plot)
        console.print(f"[green]Saved[/green] {metrics_plot}")
    else:
        console.print("[yellow]frame_metrics.csv not found, skipping metrics plot[/yellow]")

    vector_map = export_vectors_dir / "vector_map.png"
    console.print("[bold]Generated images:[/bold]")
    console.print(f"  Vector map: {vector_map}")
    if metrics_csv.exists():
        console.print(f"  Metrics:    {metrics_plot}")
