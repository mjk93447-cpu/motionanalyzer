"""Tests for DRAEM: discriminative head and synthetic anomaly training."""
import numpy as np
import pytest
...
def test_draem_with_discriminative_fit_predict() -> None:
...
@pytest.mark.skipif(not _has_torch(), reason="PyTorch not installed")
def test_draem_without_discriminative_reconstruction_score() -> None:
    from motionanalyzer.ml_models.draem import DRAEMPyTorch
...
@pytest.mark.skipif(not _has_torch(), reason="PyTorch not installed")
def test_draem_save_load_preserves_discriminative() -> None:
    import tempfile
    ...
    model = DRAEMPyTorch(input_dim=5, use_discriminative=True, batch_size=4)
    model.fit(X, epochs=2)
    scores_before = model.predict(X[:3])

    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "draem.pt"
        model.save(path)
        model2 = DRAEMPyTorch(input_dim=5)
        model2.load(path)
